...
def get_bom_component_rows(item, company=None):
	rows = frappe.get_all(
		"BOM Item",
		filters={"item_code": item, "parenttype": "BOM", "docstatus": ["<", 2]},
...
def get_product_bundle_component_rows(item):
	rows = frappe.get_all(
		"Product Bundle Item",
		filters={"item_code": item, "parenttype": "Product Bundle"},
...
def get_bom_map(bom_names, company=None):
	...
	filters = {"name": ["in", bom_names], "docstatus": ["<", 2]}
	if company:
		filters["company"] = company
